...
MapResult mapPut(Map map, const char *key, const char *data) //DONE
{
    ...
    if (mapEntryCreateOrPromote(&(map->map_tail)) == NULL){
        free(data_copy);
        free(key_copy);
...
static char *mapGetNextKeyAndPromote(MapEntry *original_entry, MapEntry *next_entry)
{
    RETURN_ON_NULL(original_entry, NULL); // Check pointers aren't NULL! bug found from elections
